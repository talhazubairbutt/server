...
class PredefinedStatusServiceTest extends TestCase {
	...
	protected function setUp(): void {
		...
		$this->service = new PredefinedStatusService($this->l10nFactory);
	}

	public function testGetDefaultStatuses(): void {
		$this->l10nFactory->expects($this->once())
			->method('get')
			->with('user_status', 'language-code42')
		...
		$actual = $this->service->getDefaultStatuses('language-code42');
		$this->assertEquals([
			[
				'id' => 'meeting',
				'icon' => '📅',
	...
	public function testGetTranslatedStatusForId(string $id, ?string $expected): void {
		...
		$actual = $this->service->getTranslatedStatusForId('language-code42', $id);
